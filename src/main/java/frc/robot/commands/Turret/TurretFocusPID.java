...
public class TurretFocusPID extends PIDCommand {
  ...
  public TurretFocusPID(Turret Turret_Inst,PIDController PID) {
    super(
      ...
      output -> {
        if (!(Turret_Inst.isTooFarLeft() && output > 0)  && !(Turret_Inst.isTooFarRight() && output < 0 )) { //ensure motor cannot turn more than fully left if headed left. Need to determine what this direction is.
          try {Turret_Inst.setPower(-output);}
